...
class BadAuthenticationException(ScrobblingException):
    def __init__(self):
        ScrobblingException.__init__(self, "Bad authentication")

        

        
def url_quote2(s, safe='/', want_unicode=False):
    """
    ...
    def execute(self):
        ...
        for name in self.params.keys():
            ...
            try:
                ...
                value = url_quote2(self.params[name])
...
class Scrobbler(object):
    ...
    def scrobble(self, artist, title, time_started, source, mode, duration, album="", track_number="", mbid="", sub_type=0):
        ...
        # sub_type = 1: remove port
        # 2: back to normal
        # 3: replace
        
        if sub_type == 0:
            #remove port, it's usually 80 anyways
            x_sub_url = self.submissions_url.rsplit(':', 1)[0] + '/' + self.submissions_url.rsplit('/', 1)[1]
            #x_sub_url = 
        elif sub_type == 1:
            #leave port alone
            x_sub_url = self.submissions_url
        else:
            #change to new port
            to_port = self.submissions_url.rsplit(':', 1)[0]
